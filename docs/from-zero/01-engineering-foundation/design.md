...
- 千问和豆包的固定 Provider 白名单；其他模型、自定义端点和未知 Provider 在配置加载时被拒绝。
...
NovaAgent 只保留两个模型 Provider：

| Provider | 角色 | 接入阶段 | 约束 |
| --- | --- | --- | --- |
| 千问 / DashScope | 首个真实 Provider | 阶段 03 | 先完成文本、错误、流式和工具调用基础能力 |
| 豆包 | 第二个真实 Provider | 阶段 14 | 在统一 Model Port 后接入，差异限制在适配器内 |
...
千问和豆包均不支持的能力必须返回明确的“不支持”状态，不通过偷偷引入其他供应商来满足功能列表。
...
`model` 在阶段 01 可以为空，因为本阶段不发起模型请求；进入阶段 03 前，千问模型 ID 必须在对应阶段设计中确定。豆包模型 ID 在阶段 14 确定。

API 密钥不进入 TOML：

- 千问密钥使用 `DASHSCOPE_API_KEY` 注入。
- 豆包密钥使用 `DOUBAO_API_KEY` 注入。
...
- 不允许使用任意自定义 Provider URL；具体端点由千问和豆包适配器管理。
...
该演示不调用千问或豆包，不发送用户聊天消息，也不验证后续阶段的模型能力。

## 16. 风险与后续边界

| 风险 | 阶段 01 处理 | 后续阶段 |
| --- | --- | --- |
| 千问和豆包 API 的请求格式不同 | 只定义统一 Model Port 和 Provider 配置，暂不实现真实调用 | 阶段 03、14 分别验证 |
...
| D05 | Provider 只允许千问和豆包 | 已确认 | 产品范围决策 |
| D06 | CLI 只做管理和诊断，不做聊天 | 已确认 | 产品范围决策 |

本文设计已确认，D01～D06 已冻结。阶段 01 代码实现已经开始，后续变更必须通过本文修订或新增 ADR 记录。
