...
4. 后续流式输出、多轮会话、工具循环和豆包接入可以继续复用同一条应用调用链，而不是另建第二套聊天实现。
...
- MOD-04：定义阶段 03 的千问文本能力声明；豆包能力保留到阶段 14 实现。
- MOD-05：继续执行千问/豆包白名单，拒绝自定义端点和第三家 Provider。
...
- 不接入豆包；豆包保留为允许的 Provider，但真实 Adapter 在阶段 14 实现。
...
- 最小页面不渲染空的“思考摘要”区域。
- 后续如果千问或豆包提供文档明确、可安全展示的摘要能力，必须在对应阶段重新设计和验收，不能仅根据字段名称直接开放。
...
| `image_input` | `false` | 阶段 14 再设计 |
| `audio_input` | `false` | 阶段 14 再设计 |
| `usage` | `true` | 运行时仍允许供应商偶发缺失 |

能力声明描述“NovaAgent 当前已经实现并验收的能力”，不直接照抄供应商营销能力。模型本身可能支持工具或多模态，也必须等对应阶段完成后才能改为 `true`。
...
- 阶段 03 的 `providers.default` 必须为 `qwen`；阶段 14 接入豆包后解除这一临时约束。
- `enabled` 仍只允许 `qwen` 和 `doubao`；阶段 03 聊天要求其中包含 `qwen`。
...
豆包配置在阶段 03 仍可被配置加载器识别和诊断，但不会被聊天服务实例化或调用。
...
| Provider | 旧系统包含多个模型入口 | 只允许千问/豆包，本阶段只调用千问 |
...
| 编号 | 推荐决策 | 状态 |
| --- | --- | --- |
| D03-01 | 阶段 03 只实现千问文本；豆包真实接入保留到阶段 14 | 已确认 |
...
| 没有真实密钥导致无法验收 | 自动化完成可控证据；最终状态保持待验收，直到负责人授权真实演示或明确决定替代证据 |
...
4. 实现完成后，真实千问 Web 演示需要负责人提供有效凭据并同意产生一次实际 API 调用；在此之前只能完成假 HTTP 自动化证据。
...
S03-01 ～ S03-11 与 S03-13 已完成。S03-12“真实千问 Web 验收”仍待项目负责人安全提供 `DASHSCOPE_API_KEY` 并授权一次真实 API 调用，或明确决定采用其他组合证据替代；在此之前阶段 03 不标记为“已完成/已验收”。
