...
本文最初用于设计确认。项目负责人已确认 D04-01 ～ D04-24，随后阶段 04 已完成实现、自动化验证和真实 Web 验收。

阶段验收后的范围变更：项目负责人确认千问为 NovaAgent 唯一模型 Provider，不再接入豆包，也不建设图片、音频、视频等模型多模态任务。该变化不影响阶段 04 已验收的千问流式与多轮行为；现有豆包配置兼容面由进度矩阵 MOD-06 跟踪清理。
...
- 用户聊天入口仍然只有 Web 控制台。
- 只允许并调用千问；阶段 04 验收代码中仍存在的豆包配置兼容面不代表产品支持。
...
- 不实现豆包或任何第二模型 Provider；该能力已永久移出路线。
- 不实现图片、音频、视频或其他模型多模态输入；该能力已永久移出路线。文本文件仍可在后续工作空间和工具阶段单独设计。
...
| 阶段 04 范围膨胀 | 明确排除工具、持久化和 Markdown；豆包与模型多模态能力已永久移出总体路线 |
...
| 编号 | 决策 | 推荐答案 | 状态 |
| --- | --- | --- | --- |
| D04-01 | 浏览器流式协议 | 使用 POST + `fetch` 读取 SSE，不使用 WebSocket 或 EventSource | 已确认 |
| D04-02 | SSE 数据格式 | 每帧只承载阶段 02 AgentEvent JSON，不建立第二套流事件 | 已确认 |
| D04-03 | 阶段 03 API | 保留 `POST /api/v1/chat` 非流式兼容路径 | 已确认 |
| D04-04 | 会话存储 | 阶段 04 只使用进程内内存存储，重启清空 | 已确认 |
| D04-05 | 持久化 | SQLite、恢复和长期历史进入阶段 10 | 已确认 |
| D04-06 | 会话并发 | 同会话只允许一个活动 run，不排队；不同会话可并发 | 已确认 |
| D04-07 | 陈旧客户端 | 使用 `expected_revision` 和 HTTP 409 防止覆盖 | 已确认 |
| D04-08 | 历史提交 | 只在成功完成后原子提交 user/assistant 对 | 已确认 |
| D04-09 | 失败和取消部分文本 | 当前页面临时显示但不进入正式历史 | 已确认 |
| D04-10 | 用户主动取消 | `POST /api/v1/runs/{run_id}/cancel`，返回 202 | 已确认 |
| D04-11 | 客户端断开 | 取消上游请求，不在后台继续生成 | 已确认 |
| D04-12 | 流式重试 | 首个文本 delta 前有限重试，之后禁止自动重试 | 已确认 |
| D04-13 | 上下文轮次 | 默认最近 20 个完整成功轮次 | 已确认 |
| D04-14 | 上下文预算 | 默认 24,000 estimated tokens，使用可替换估算 Port | 已确认 |
| D04-15 | 上下文裁剪 | 从最旧完整轮次开始，当前输入绝不静默截断 | 已确认 |
| D04-16 | 系统提示词 | 阶段 04 默认无系统消息，只预留服务端注入点 | 已确认 |
| D04-17 | 裁剪可观测性 | 新增不含正文的 `context_prepared` AgentEvent | 已确认 |
| D04-18 | 会话容量 | 每进程最多 100 个，不自动淘汰 | 已确认 |
| D04-19 | SSE 背压 | 每连接容量 64 的有界队列 | 已确认 |
| D04-20 | 断线恢复 | 阶段 04 不回放事件；断线即取消，用户重试 | 已确认 |
| D04-21 | 前端实现 | 继续原生 HTML/CSS/JavaScript，不增加构建链 | 已确认 |
| D04-22 | 输出渲染 | 继续纯文本，Markdown 留到阶段 07 | 已确认 |
| D04-23 | 密钥管理 | 继续使用 `.env`/服务端环境，Web 永不管理 Provider 密钥 | 已确认 |
| D04-24 | 阶段验收 | 自动化 + MockTransport + 真实千问双会话流式演示 | 已确认 |

## 23. 当前结论

阶段 03 已通过真实 Web 验收，阶段 04 的前置条件已经满足。本文已经为 SSE、会话状态、revision、原子提交、取消、断开、上下文预算、错误边界、安全规则和验收方式给出推荐答案，没有把架构问题留成无建议的空白项。

项目负责人已确认 D04-01 ～ D04-24，阶段 04 已完成实现、自动化测试和真实 Web 验收。当前结论如下：

1. 本文状态为“设计已确认；已实现；已验收”。
2. 总体路线和进度矩阵中的阶段 04 状态已同步为“已完成/已验收”。
3. 同目录 `completion-report.md` 已记录自动化、MockTransport 和真实 Web 验收证据。
